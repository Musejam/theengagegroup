 ...
 * Description: Chat with your customers on your site for sales and support easily, and beautifully.
 * Version: 1.2.1
 * Author: Screets
...
class SC_Chat {
	 ...
	 * @var array
	 */
	var $sc_chat_default_opts = array();
	...
	function init() {
		...
		// Variables
		$this->skin_url = apply_filters( 'sc_chat_skin_url', 'skins/' . $this->opts['default_skin'] );
	...
	function show_chatbox() {
		...
		// Show skin file
		if(
			( $this->opts['hide_chat_when_offline'] == 0 or $this->any_online_op ) 
			and	( 
			...
			or ( $this->opts['always_show_homepage'] == 1 and ( $this->opts['hide_chat_when_offline'] == 0 or $this->any_online_op ) and ( is_home() or is_front_end() ) )
	...
	function upgrade( $current_version ) {
		...
		if( version_compare( $current_version, '1.2', '<') or empty( $current_version ) ) {
