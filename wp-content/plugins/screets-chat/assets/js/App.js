	...
	var chat = {
		...
		init : function () {
			...
			if (!sc_chat.is_admin) {
				...
				if (sc_chat.use_css_anim == 1)
					setTimeout(chat.animate_chatbox, (sc_chat.delay * 500)); // CSS anims is waiting a little bit
				else
					setTimeout(chat.animate_chatbox, (sc_chat.delay * 1000));
			...
			$(document).on('click', '.sc-chat-btn-logout', function () {
				...
				$.sc_POST('logout', 'action=sc_chat_ajax_callback');
			 ...
			 */
				...
				// Directly open chatbox without displaying login form
					...
					if ($.cookie('sc_chat_chatbox_status') == 'on') {
						...
						chat.data.sc_chat_box_visible = true;
					}
		...
		open_new_tab : function (receiver_ID, visitor_ID, force_focus) {
			...
			if ($('.sc-chat-tabs li#Tab_' + receiver_ID).length == 0) {
				...
				$('.sc-chat-tabs').append(chat.render('new_tab_title', data));

				// Insert new tab content
				$('.sc-chat-tab-contents').append(chat.render('new_tab_content', data));
		...
		get_user_info : function (visitor_ID, receiver_ID) {
			...
			$.sc_GET('user_info', 'action=sc_chat_ajax_callback&ID=' + visitor_ID, function (r) {
				...
				if (r.device_name != 'null') {
					...
					chat.data.user_info = r.device_name + ' ' + r.device_version + ' - ' + r.platform + ', ' + r.ip_address + ' &nbsp; <a href="admin.php?page=sc_chat_m_chat_logs&action=edit&visitor_ID=' + visitor_ID + '" target="_blank">' + sc_chat.tr_chat_logs + '</a>';
		...
		update_user_info : function () {
			...
			// Update user info
			$('#Receiver_' + chat.data.receiver_ID + ' .sc-chat-user-agent').html(chat.data.user_info);
		...
		add_chat_line : function (params) {
			...
			if (sc_chat.is_admin == true) {
				...
				if (params.author == sc_chat.username && params.receiver_ID == sc_chat.username)
					chat.data.sender = sc_chat.username;
		...
		get_chat_lines : function (callback) {
			...
			$.sc_GET('get_chat_lines', {
				...
				if (r.chats.length) {
					...
					if (chat.data.first_time != true && (chat.data.win_is_active == 0 || sc_chat.is_admin == true))
		...
		animate_chatbox : function () {
			...
			if (sc_chat.use_css_anim == 1) {
				...
				sc_chat_header_obj.click(function () {
					...
					// Open it completely
					if (chat.data.sc_chat_box_visible == false) {
					...
					// Hide Chat Box
						...
						chat.data.sc_chat_box_visible = false;
					}
				});
		...
		play_sound : function (sound_name) {
			...
			chat.add_source(audio, sc_chat.plugin_url + '/assets/sounds/' + sound_name + '.mp3');
			chat.add_source(audio, sc_chat.plugin_url + '/assets/sounds/' + sound_name + '.ogg');
			chat.add_source(audio, sc_chat.plugin_url + '/assets/sounds/' + sound_name + '.wav');
			audio.appendTo('body');
