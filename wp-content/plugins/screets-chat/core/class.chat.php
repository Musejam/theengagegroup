...
class Chat {
	...
	public static function send_contact_from( $data ) {
		...
		// Prepare data
		$to = $sc_opts['offline_msg_email'];
		$subject = sprintf( __( '%s wrote a note', 'sc_chat' ), $data['f_chat_user_email'] );
	...
	public static function login( $name, $email, $is_admin = false ) {
		...
		$_SESSION['chat_user_name'] = $user['name'];
		$_SESSION['chat_user_gravatar'] = $user['gravatar'];
		$_SESSION['chat_user_email'] = $user['email'];
	...
	public static function is_user_logged_in() {
		...
		if( !empty( $_SESSION['chat_user_name'] ) ) {
			...
			$response['user'] = array(
				'name'		=> $_SESSION['chat_user_name'],
				'email'		=> $_SESSION['chat_user_email']
			);
	...
	public static function online() {
		...
		$chat_user_name = $_SESSION[ 'chat_user_name' ];
	...
	public static function send_chat_msg( $params ) {
		...
		$chat_user_name = $_SESSION[ 'chat_user_name' ];
		$chat_user_gravatar = $_SESSION[ 'chat_user_gravatar' ];
		$chat_user_email = $_SESSION[ 'chat_user_email' ];
	...
	public static function get_online_users() {
		...
		if( isset( $_SESSION['chat_user_name'] ) ) {
			...
			$user = new Chat_user( array(
				'name' 		=> $_SESSION['chat_user_name'],
				'email'		=> $_SESSION['chat_user_email'],
				'gravatar'	=> $_SESSION['chat_user_gravatar'],
				'type'		=> $user_type
			));
		...
		if( current_user_can('chat_with_users') ) {
			...
			foreach( $online_users as $user ) {
				...
				if( $user->type == 1 and @$_SESSION['chat_user_name'] == $user->name )
