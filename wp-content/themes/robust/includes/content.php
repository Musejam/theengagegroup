<article id="post-<?php the_ID(); ?>" <?php post_class('entry clearfix'); ?>>
	...
	<div class="entry-badge">
	    <a href="<?php the_permalink(); ?>"><i class="icon-pencil"></i></a>
	</div>

	<?php if( (function_exists('has_post_thumbnail')) && has_post_thumbnail() ) : ?>
		<figure class="entry-asset">
			<?php if( !is_singular() ) : ?>
		    	<a href="<?php the_permalink(); ?>" title="<?php printf( esc_attr__( 'Permalink to %s',  THEME_FX ), the_title_attribute( 'echo=0' ) ); ?>" rel="bookmark">
		    		<?php echo get_the_post_thumbnail( $post->ID, '548x300' ); ?>
		    	</a>
			<?php else : ?>
				<?php echo get_the_post_thumbnail( $post->ID, '548x300' ); ?>
			<?php endif; ?>
