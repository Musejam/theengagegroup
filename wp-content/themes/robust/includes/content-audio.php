<article id="post-<?php the_ID(); ?>" <?php post_class('entry clearfix'); ?>>
	...
	<div class="entry-badge">
	    <a href="<?php the_permalink(); ?>"><i class="icon-volume-up"></i></a>
	</div>

	<div class="entry-asset asset-audio">
        <?php 
        $format = get_post_meta( $post->ID, 'ft_audio_format', true );
        $file = get_post_meta( $post->ID, 'ft_audio_file', true );
        $optional = get_post_meta( $post->ID, 'ft_audio_optional', true );
        echo freshthemes_print_audio_player($post->ID, $format, $file, $optional); 
        ?>
    </div>
